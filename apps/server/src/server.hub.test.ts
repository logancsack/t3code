 ...
 * The machine directory points nowhere: nothing here wakes a thread machine.
 */
...
const hubConfig = (schema: HubTestSchema, baseDir: string) =>
  Effect.gen(function* () {
    ...
    return {
      ...config,
      // Nothing here wakes a thread machine; the directory is never called.
      hub: { ...config.hub, machinesUrl: "http://127.0.0.1:9", machinesToken: "unused" },
      host: "127.0.0.1",
      port: 0,
