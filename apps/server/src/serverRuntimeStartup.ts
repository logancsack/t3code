...
import * as TurnLivenessWatchdog from "./orchestration/Services/TurnLivenessWatchdog.ts";
import {
...
export const make = (options?: StartupOptions) =>
  Effect.gen(function* () {
    ...
    const turnLivenessWatchdog = yield* TurnLivenessWatchdog.TurnLivenessWatchdog;
    ...
    const startup = Effect.gen(function* () {
      ...
      yield* runStartupPhase(
        ...
        Effect.gen(function* () {
          ...
          yield* turnLivenessWatchdog.start().pipe(Scope.provide(reactorScope));
        }),
      );
