...
import { ProjectionThreadMessage } from "../Services/ProjectionThreadMessages.ts";
import { ProjectionThreadProposedPlan } from "../Services/ProjectionThreadProposedPlans.ts";
import { ProjectionThreadSession } from "../Services/ProjectionThreadSessions.ts";
import { ProjectionThread } from "../Services/ProjectionThreads.ts";
import {
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  // snapshotSequence and a resuming client would skip events. REPEATABLE READ
  // restores the SQLite guarantee; it must be the transaction's first statement.
  const withSnapshotTransaction = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
    sql.withTransaction(
    ...
    execute: () =>
      sql`
        ...
        INNER JOIN projection_threads threads
          ON threads.user_id = sessions.user_id
          AND threads.thread_id = sessions.thread_id
    ...
    execute: () =>
      sql`
        ...
        JOIN projection_turns turns
          ON turns.user_id = threads.user_id
  ...
  const readProjectionCounts = SqlSchema.findOne({
    ...
    execute: () =>
      sql`
        SELECT
          (SELECT COUNT(*) FROM projection_projects WHERE user_id = ${userId}) AS "projectCount",
          (SELECT COUNT(*) FROM projection_threads WHERE user_id = ${userId}) AS "threadCount"
      `,
  });
      ...
      sql`
        WITH ranked AS (
          ...
          INNER JOIN projection_threads AS threads
            ON threads.user_id = messages.user_id
          ...
          WHERE messages.user_id = ${userId}
            ...
            AND messages.text ILIKE ${pattern} ESCAPE '!'
        )
        SELECT
          thread_id AS "threadId",
          project_id AS "projectId",
  ...
  const listThreadActivityRowsByIds = SqlSchema.findAll({
    ...
    execute: ({ activityIds }) =>
      sql`
        ...
        WHERE user_id = ${userId}
          AND ${sql.in("activity_id", activityIds)}
      `,
  });
    ...
    execute: ({ threadId, beforeAnchorAt, beforeTurnKey, userTurnLimit, maxRawTurns }) =>
      sql`
        WITH candidates AS (
          ...
          ORDER BY turns.requested_at DESC, turns.turn_id DESC NULLS LAST
          LIMIT ${maxRawTurns}
        ),
        walked AS (
          SELECT
          ...
          LEFT JOIN projection_thread_messages AS messages
            ON messages.user_id = ${userId}
            AND messages.message_id = candidates.pending_message_id
        )
        SELECT
  ...
  // SQLite json_extract accepts the "\u0000" escapes JSON.stringify emits for NUL
  // characters (13 real activity payloads in the owner's history have them);
  // Postgres json/jsonb parsing rejects them. Neutralize the escape before
  // parsing: only requestId/detail are extracted and neither contains it.
  const activityPayloadJson = sql.literal(
    String.raw`replace(activity.payload_json, '\u0000', '\ufffd')::json`,
  );
...
pending_approval_requests AS (
        ...
        pending_approval_activities AS (
          ...
          WHERE activity.user_id = ${userId}
            AND activity.thread_id = pending.thread_id
            AND activity.kind = 'approval.requested'
            AND (${activityPayloadJson} ->> 'requestId') = pending.request_id
        ),
        ...
        user_input_lifecycle AS (
          SELECT
            ...
            activity.kind,
            ROW_NUMBER() OVER (
              PARTITION BY (${activityPayloadJson} ->> 'requestId')
              ORDER BY activity.created_at DESC, activity.activity_id DESC
            ) AS request_order
          ...
          WHERE activity.user_id = ${userId}
            ...
            AND (
              ...
              OR (
                ...
                AND (
                  lower(COALESCE(${activityPayloadJson} ->> 'detail', ''))
                    LIKE '%stale pending user-input request%'
                  OR lower(COALESCE(${activityPayloadJson} ->> 'detail', ''))
            ...
            AND (${activityPayloadJson} ->> 'requestId') IS NOT NULL
        ),
    ...
    execute: ({ threadId }) =>
      sql`
        ...
        INNER JOIN projection_thread_activities AS activity
          ON activity.user_id = ${userId}
          AND activity.activity_id = pinned.activity_id
      ...
      sql`
        SELECT
          ...
          (
            ...
            FROM projection_turns AS turns
            WHERE turns.user_id = threads.user_id
              AND turns.thread_id = threads.thread_id
...
export const PgProjectionSnapshotQueryLive = Layer.effect(
  ProjectionSnapshotQuery,
