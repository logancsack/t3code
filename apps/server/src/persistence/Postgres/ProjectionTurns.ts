  ...
  const upsertProjectionTurnById = SqlSchema.void({
    ...
    execute: (row) =>
      sql`
        ...
        ON CONFLICT (user_id, thread_id, turn_id)
        DO UPDATE SET
    ...
    execute: ({ threadId }) =>
      sql`
        ...
        ORDER BY
          ...
          requested_at ASC,
          -- SQLite sorts NULL first; Postgres needs it spelled out.
          turn_id ASC NULLS FIRST
      `,
  });
...
export const PgProjectionTurnRepositoryLive = Layer.effect(
  ProjectionTurnRepository,
