...
const makeEventStore = Effect.gen(function* () {
  ...
  // SQLite's global AUTOINCREMENT becomes a per-user counter. The counter row is
  // locked by the append transaction, so appends for one user serialize and the
  // sequence stays gap-free (a rolled-back append rolls the counter back too).
    ...
    execute: (request) =>
      sql`
        WITH next_sequence AS (
          ...
          VALUES (${userId}, 1)
          ON CONFLICT (user_id)
          DO UPDATE SET last_event_sequence = hub_users.last_event_sequence + 1
        ...
        VALUES (
          ${userId},
          (SELECT last_event_sequence FROM next_sequence),
          ${request.eventId},
          ...
          COALESCE(
            (
              ...
              WHERE user_id = ${userId}
                AND aggregate_kind = ${request.aggregateKind}
...
export const PgOrchestrationEventStoreLive = Layer.effect(OrchestrationEventStore, makeEventStore);
