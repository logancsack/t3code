...
export const ComposerFooterModeControls = memo(function ComposerFooterModeControls(props: {
  ...
  const supportedRuntimeModes =
    props.supportedRuntimeModes.length > 0 ? props.supportedRuntimeModes : runtimeModeOptions;
      ...
      <Tooltip>
        ...
        >
          <TooltipTrigger
            render={
              <SelectTrigger
                ...
                disabled={isRuntimeModeFixed}
                aria-label={
                  isRuntimeModeFixed
                    ? `Runtime mode: ${runtimeModeOption.label} (required by provider)`
                    : "Runtime mode"
                }
              />
            }
          >
            ...
            <SelectValue>
              {isRuntimeModeFixed
                ? `${runtimeModeOption.label} · Required`
        ...
        <TooltipPopup side="top">
          {isRuntimeModeFixed
            ? `This provider requires ${runtimeModeOption.label}.`
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const addComposerAttachments = (files: File[]) => {
    ...
    const nextImages: ComposerAttachment[] = accepted.map((attachment) => ({
      ...attachment,
      id: randomUUID(),
      // Only images have something to show. Minting an object URL for a zip
      // would leak a blob the composer never revokes and never renders.
      previewUrl: attachment.type === "image" ? URL.createObjectURL(attachment.file) : "",
    }));
