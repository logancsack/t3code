...
function deriveTurnFolds(input: {
  ...
  for (const [turnId, group] of groupsByTurnId) {
    ...
    for (const entry of group.entries) {
      ...
      if (entry.kind === "work" && workLogEntryHasGeneratedImage(entry.entry)) {
        continue;
      }
...
export function deriveMessagesTimelineRows(input: {
  ...
  // A waking managed machine is not thinking yet; the working row says so.
  if (input.isWorking && !hasActivityRow && !input.isWakingWorkspace) {
    nextRows.push({
      kind: "thinking",
  ...
  switch (a.kind) {
    ...
    case "work": {
      ...
      return (
        ...
        workGroupsAreUnchanged(a.groupedEntries, bw.groupedEntries)
      );
    }
