...
export function AuthConnectorDialog(props: {
  ...
  }, [environmentId, getConnector, open, session?.fields.length, session?.id, session?.status]);
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    ...
    if (environmentId && session && (session.status === "starting" || session.status === "waiting")) {
