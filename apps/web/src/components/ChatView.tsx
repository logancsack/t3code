...
function ChatViewContent(props: ChatViewProps) {
  ...
  const systemComposerBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
    ...
    const unavailableConnection = activeEnvironmentActionUnavailableState?.connection ?? null;
  ...
  // The plan step the turn is working on (an in-progress step, else the first
  // pending one) labels the working row alongside the elapsed time.
  ...
  const onSend = async (
    ...
    if (activeEnvironmentActionUnavailable) {
      toastManager.add(
        stackedThreadToast({
          type: "warning",
