...
export const PersistedComposerAttachment = Schema.Struct({
  ...
  // Optional because drafts persisted before the composer accepted non-image
  // files have no `type` at all; those are images by construction.
 ...
 *
 * `type` mirrors the wire contract: `image` attachments become native content
 * blocks for the model, `file` attachments are written to disk on the server and
 * handed to the agent as a path. Only images have a renderable `previewUrl`;
 * for files it is the empty string and the composer shows a chip instead.
 */
...
function normalizePersistedAttachment(value: unknown): PersistedComposerAttachment | null {
  ...
  // Drafts written before the composer accepted non-image files carry no
  // `type`; anything unrecognized is treated as an image, which is what those
  // drafts always were.
    ...
    return [
      {
        ...
        // A file attachment has nothing to render, and handing its data URL to
        // an <img> would only produce a broken image.
